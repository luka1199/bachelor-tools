...
async function run() {
    ...
    if (process.argv[5] != null) {
        console.log(`Saving module keywords to ${process.argv[4]}`);
        fs.writeFileSync(process.argv[5], JSON.stringify(moduleKeywords, null, '\t'))
    }
}
...
function getData(testScripts, keywords) {
    ...
    for (let j = 0; j < testScripts.length; j++) {
        ...
        for (let k = 0; k < keywords.length; k++) {
            ...
            if (script.includes(keyword)) {
                ...
                if (!moduleKeywords[moduleName].includes(keyword))
                moduleKeywords[moduleName].push(keyword)
            }
        }
    ...
    return [keywordCounter, modulesWithTestScript, moduleKeywords]
}
